            ...
            match tx.status {
                ...
                TransactionStatus::Failed(ref reason) => {
                    ...
                    if tx.retry_count < MAX_RETRY_ATTEMPTS {
                        ...
                        error!("Transaction {} exceeded max retry attempts: {}", tx.txid, reason);
                        to_remove.push(idx);
                    }
                },
    ...
    // Second pass: remove transactions
    {
        ...
        for idx in to_retry {
            if idx < pending.len() {
                ...
                debug!("Preparing to retry transaction {}, attempt {}", tx.txid, tx.retry_count + 1);
        ...
        for idx in to_remove {
            if idx < pending.len() {
                ...
                if verbose_logging {
                    debug!("Removed transaction {} from queue, status: {:?}", tx.txid, tx.status);
                }
            }
        }
    }
...
pub fn add_transaction_to_pending_queue(
    ...
    if verbose_logging {
        debug!("Added transaction to pending queue. Queue size: {}", pending.len());
    }
}
...
pub fn collect_pending_transactions(
    ...
    if verbose_logging && !transactions.is_empty() {
        debug!("Collected {} pending transactions for status check", transactions.len());
    }
